...
(define (display-legend game-state)
  (displayln "^ - You are here")
  (for-each (λ (location-name) (displayln location-name))
            (map (λ (num loc) (string-append (number->string num) " - " (location-name loc))) (range 1 (add1 (length (game-map game-state)))) (game-map game-state))))
